...
class ANNMemory():
    def __init__(
        ...
        self.anns = [ANN(dim = dim, max_num_entries = max_memories, use_gpu = ann_use_gpu, cap_num_entries = True) for _ in range(num_indices)]
    ...
    def search(self, queries, topk, nprobe = 8):
        ...
        for ind, (query, ann) in enumerate(zip(queries, self.anns)):
            indices = ann.search(query, topk, nprobe)
            mask = indices !=  -1
